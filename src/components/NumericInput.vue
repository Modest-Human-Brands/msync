...
<template>
  <div class="flex items-center gap-1 bg-black/60 border border-white/10 rounded px-2 py-0.5"
    :class="readonly ? 'opacity-50' : ''">
    <span class="text-2xs text-white/30 w-3 shrink-0 select-none font-medium">{{ label }}</span>
    <input type="number" :value="modelValue" :min="min" :max="max" :step="step ?? 1" :readonly="readonly"
