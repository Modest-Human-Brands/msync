...
const processing = ref({ total: 0, processed: 0, currentFile: '', startedAt: 0, errors: [] as string[] })
...
const { pause: pauseTimer, resume: resumeTimer } = useIntervalFn(() => { now.value = Date.now() }, 200, { immediate: false })

const selectedMedia = computed(() => mediaFiles.value[selectedIndex.value])
const hasSelection = computed(() => mediaFiles.value.length > 0)
const isValid = computed(() => srcDir.value.trim() !== '' && destDir.value.trim() !== '' && watermarkPath.value.trim() !== '')

const pct = computed(() => progressState.value.total > 0 ? Math.round((progressState.value.processed / progressState.value.total) * 100) : 0)
...
const watermarkStyle = computed(() => {
  const base: Record<string, string> = {
    position: 'absolute', width: `${wmSize.value}%`, opacity: String(wmOpacity.value), pointerEvents: 'none', userSelect: 'none'
  }
...
const isImageFile = (item?: Pick<MediaFile, 'type' | 'name'>) => !!item && (item.type.startsWith('image/') || /\.(png|jpe?g|webp|gif|bmp|svg)$/i.test(item.name))
...
async function run() {
  ...
  try {
    ...
    result.value = { total: res.total, errors: res.errors.length, elapsedMs: Date.now() - startedAt }
  } catch (err) {
    console.error('Watermark process failed:', err)
    result.value = { total: progressState.value.processed, errors: 1, elapsedMs: Date.now() - startedAt }
  }
...
<template>
  <ToolLayout v-model:query="query" @submit:query="onSubmit" @action="(v) => v === 'enter' && onBrowse()">
    ...
    <div v-if="step === 'configure' && !hasSelection" @dragover.prevent
        ...
        <div
          ...
          <template v-else>
            <div class="text-center text-gray-500">
              ...
              <div class="text-xs mt-1">{{ selectedMedia?.name || 'Select a file on the left' }}</div>
            </div>
          </template>
        </div>
          ...
          <div class="bg-white/5 rounded-xl border border-white/8 p-4 space-y-4 min-w-0">
            <div class="flex flex-col gap-1.5">
              ...
              <div class="flex gap-1.5">
                <div
                  ...
                  :title="watermarkPath || 'Click to browse'" @click="browseWatermark">
                ...
                <div
                  ...
                  <button v-for="p in POSITIONS" :key="p.value" :style="{ gridColumn: p.col, gridRow: p.row }"
                    :title="p.value" :class="[
                      ...
                      position === p.value ? 'bg-orange-500 text-white' : 'text-gray-600 hover:text-gray-300 hover:bg-white/10',
            ...
            <div class="flex flex-col gap-1.5">
              ...
              <div class="flex gap-1.5">
                <div
                  class="flex-1 min-w-0 bg-white/5 border border-white/8 rounded-md px-2.5 py-1.5 text-xs text-white/80 truncate cursor-pointer hover:bg-white/8 transition"
                  :title="destDir || 'Auto-filled when source is picked'" @click="browseOutput">
            ...
            <button :disabled="!isValid" :class="[
              ...
              isValid ? 'bg-orange-500 hover:bg-orange-400 text-white cursor-pointer' : 'bg-white/5 text-gray-600 cursor-not-allowed',
            ]" @click="run">
    ...
    <div v-else-if="step === 'processing'" class="h-full flex flex-col items-center justify-center gap-5">
      ...
      <div class="text-center">
        ...
        <div class="text-xs text-gray-400 mt-1">{{ progressState.processed }} / {{ progressState.total }} files</div>
      </div>
      ...
      <div class="flex items-center justify-between w-56 text-2xs text-gray-500">
        <span class="truncate max-w-[75%]" :title="progressState.currentFile">{{ progressState.currentFile || '—'
        }}</span>
    ...
    <div v-else-if="step === 'done' && result" class="h-full flex flex-col items-center justify-center gap-5">
      ...
      <div class="text-center">
        <div class="text-sm font-semibold text-white">{{ result.errors === 0 ? 'Done!' : 'Completed with errors' }}
        </div>
        <div class="text-xs text-gray-400 mt-1">{{ result.total }} file{{ result.total !== 1 ? 's' : '' }} · {{
          formatMs(result.elapsedMs) }}</div>
      </div>
