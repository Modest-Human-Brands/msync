...
const props = withDefaults(
  defineProps<{ query?: string; preventEscape?: boolean }>(),
  { query: '', preventEscape: false },
)
...
<template>
  <main class="flex flex-col w-screen h-screen bg-dark-400 text-white overflow-hidden" tabindex="0">
    <div class="border-b border-white/10 px-5 py-3 flex items-center gap-3 shrink-0">
      <slot name="header">
        <input :value="query" @input="(e) => emit('update:query', (e.target as HTMLInputElement).value)"
    ...
    <div class="border-t border-white/10 px-5 py-3 flex items-center justify-between shrink-0">
      <slot name="footer">
        ...
        <button class="bg-white/10 px-2 py-1 rounded text-xs text-white hover:bg-white/20"
