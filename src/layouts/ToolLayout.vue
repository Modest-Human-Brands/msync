...
<template>
  <main class="flex flex-col w-screen h-screen bg-[#1e1e1e]/95 text-white" tabindex="0">
    ...
    <div class="px-5 py-4 border-b border-white/10 flex items-center gap-3">
      <input :value="query" @input="event => emit('update:query', event.target.value)"
        @keydown.enter="emit('submit:query')" placeholder="Type a command or input..."
    ...
    <!-- Footer -->
    <div class="px-5 py-3 border-t border-white/10 flex justify-between items-center text-xs text-gray-400">
      <span>ESC to go back</span>
      <span class="bg-white/10 px-2 py-1 rounded-md text-white text-xs">
        Enter ↵
      </span>
    </div>
  </main>
